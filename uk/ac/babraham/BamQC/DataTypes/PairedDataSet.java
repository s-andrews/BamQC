...
import uk.ac.babraham.BamQC.Utilities.NonThreadSafeIntCounter;
import uk.ac.babraham.BamQC.Utilities.ThreadSafeIntCounter;
...
public class PairedDataSet extends DataSet implements HiCDataStore {
	...
	/* (non-Javadoc)
	 * @see uk.ac.babraham.BamQC.DataTypes.DataStore#isHiC()
	 */
	 ...
	 * 
	 * Only the BamQC parser should ever set the noReverse parameter to true.  Specifying
	...
	public void addData (Chromosome c, long read, boolean skipSorting) {
		...
		// Skip cis reads which are too close together
			...
			try {
				...
				if (isFinalised) {
					throw new BamQCException("This data set is finalised.  No more data can be added");
				}		
...
//					System.err.println("Finished forward tracking at index "+i+" position "+SequenceRead.midPoint(sourceReads[i]));
	...
	public synchronized HiCHitCollection getExportableReadsForChromosome(Chromosome c) {
		...
		for (int i=0;i<chrs.length;i++) {
			...
			Chromosome thisChr = BamQCApplication.getInstance().dataCollection().genome().getChromosome(chrs[i]).chromosome();
	...
	/*
	 * (non-Javadoc)
	 * @see uk.ac.babraham.BamQC.DataTypes.HiCDataStore#getCisCountForChromosome(uk.ac.babraham.BamQC.DataTypes.Genome.Chromosome)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see uk.ac.babraham.BamQC.DataTypes.HiCDataStore#getTransCountForChromosome(uk.ac.babraham.BamQC.DataTypes.Genome.Chromosome)
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see uk.ac.babraham.BamQC.DataTypes.HiCDataStore#getCisCount()
	 */
	@Override
	...
	/*
	 * (non-Javadoc)
	 * @see uk.ac.babraham.BamQC.DataTypes.HiCDataStore#getTransCount()
	 */
	@Override
	...
	private class ChromosomeDataStore implements Runnable {
		...
		public void run() {
			...
			try {
				...
				tempFile = File.createTempFile("seqmonk_data_hic", ".temp", BamQCPreferences.getInstance().tempDirectory());
