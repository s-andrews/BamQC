...
public class DataCollection {
	 ...
	 */
	public void setActiveDataStore (DataStore d) throws BamQCException {
		if (d == null || dataSets.contains(d) || dataGroups.contains(d) || replicateSets.contains(d)) {
			...
			throw new BamQCException("Data store "+d.name()+" could not be found in the data collection");
		}
	}
	...
	public DataStore getActiveDataStore () {
		...
		else if (BamQCApplication.getInstance().drawnDataStores().length == 1) {
			return BamQCApplication.getInstance().drawnDataStores()[0];
		}
		else {
			return null;
		}
