...
import uk.ac.babraham.BamQC.Displays.HTMLDisplay.HTMLDisplayDialog;
...
public class CrashReporter extends JDialog implements ActionListener {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 1994818786192552431L;
...
//	public static final String reportURLString = "http://bilin1/cgi-bin/public/crashreporter.cgi";
	...
	public CrashReporter (Throwable e) {
		super(BamQCApplication.getInstance(),"Oops - Crash Reporter");
		...
		if (e instanceof OutOfMemoryError) {
			...
			JOptionPane.showMessageDialog(BamQCApplication.getInstance(), "<html>You ran out of memory!<br><br>Please look at Help &gt; Contents &gt; Configuration to see how to fix this", "Out of memory", JOptionPane.ERROR_MESSAGE);				
			return;
		}
		...
		ImageIcon sadMonk = new ImageIcon(ClassLoader.getSystemResource("uk/ac/babraham/BamQC/Resources/sad_monk100.png"));
		...
		JLabel genericMessage = new JLabel("BamQC encountered a problem.",JLabel.CENTER);
	...
	private class ReportSender extends JDialog implements ActionListener, Runnable {
		 ...
		 * 
		 */
		private static final long serialVersionUID = 1982942487179653706L;
		...
		/** A flag to say if we found any BamQC specific classes **/
		private boolean foundBamQCClass = false;
		...
		public ReportSender (CrashReporter cr) {
			...
			email.setText(BamQCPreferences.getInstance().getCrashEmail());
			...
			if (BamQCPreferences.getInstance().getCrashEmail().length() > 0) {
		...
		private String makeReportText () {
			...
			sb.append("BamQC Version:");
			sb.append(BamQCApplication.VERSION);
			sb.append("\n\n");
			...
			if (e.getClass().getName().indexOf("BamQC") >=0) {
				foundBamQCClass = true;
			}
			sb.append("\n");
			sb.append(e.getMessage());
			if (e.getMessage() != null && e.getMessage().indexOf("BamQC") >=0) {
				foundBamQCClass = true;
			}
			sb.append("\n\n");
			...
			for (int i=0;i<elements.length;i++) {
				...
				sb.append("\n");
				if (elements[i].toString().indexOf("BamQC") >=0) {
					foundBamQCClass = true;
				}
			}
		...
		public void actionPerformed(ActionEvent ae) {
			if (ae.getActionCommand().equals("cancel")) {
				...
				if (! foundBamQCClass) {
					int reply = JOptionPane.showConfirmDialog(this, "<html>This error doesn't appear to come from within BamQC but is a bug in the core Java classes.<br>You can still submit this report, but the BamQC authors may not be able to fix it!<br><br>Do you still want to send the report?</html>", "Not our fault!", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE);
				...
				if (rememberEmail.isSelected()) {
					BamQCPreferences.getInstance().setCrashEmail(email.getText());
				}
				else {
					BamQCPreferences.getInstance().setCrashEmail(null);
				}
				
				try {
					BamQCPreferences.getInstance().savePreferences();
				} 
				catch (IOException e) {
		...
		public void run() {
			...
			// BamQCPreferences class and should be picked up
			// automatically.
